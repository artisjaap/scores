...
import {addDart, clearCurrentDartSet, commitScore} from './darts-scoreboard-the-scoreboard.actions';
...
function calcuateEndingsForScore(score: number) {
  switch (score){
    ...
    case 113:	return [['T19', '16', 'D20']] ;
    case 112:	return [['T20', '20', 'D16']] ;
    case 111: return [['T20', '19', 'D16']];
    case 110: return [['T20', '18', 'D16']];
    case 109: return [['T19', '20', 'D16']];
    case 108: return [['T20', '16', 'D16']];
    case 107: return [['T19', '18', 'D16']];
    case 106: return [['T20', '14', 'D16']];
    case 105: return [['T19', '16', 'D16']];
    case 104: return [['T18', '18', 'D16']];
    case 103: return [['T20', '3', 'D20']];
    case 102: return [['T16', '14', 'D20']];
    case 101: return [['T20', '1', 'D20']];
    ...
    case 81: return [['T19', 'D12']];
    case 80: return [['T20', 'D10']];
    case 79: return [['T19', 'D11']];
    ...
    case 	69:	return [['T19', 'D6']];
    case 	68:	return [['T20', 'D4']];
    case 	67:	return [['T17', 'D8']];
    case 	66:	return [['T14', 'D12']];
...
function addScore(player: DartsPlayer, darts: string[]) {
  ...
  if(player.currentPlayer){
    return {
      ...
      doubleEndings: calcuateEndingsForScore(latestScore - calculatedScore)
    }
...
function intermediateEnding(state: TheDartsScoreboardState, darts : string[], player: DartsPlayer):  String[][] {
  let intermediateScore = calcScore(darts);
  let latestScore: number = player.score[player.score.length-1].newScore;
  let endings:String[][] = calcuateEndingsForScore(latestScore - intermediateScore);
  let dartsLeft = 3 - darts.length;
  return endings.filter(ending => ending.length <= dartsLeft);
}

const createTheDartsScoreboardReducer = createReducer(
  theDartsScoreboardInitialState,
  on(addDart, (state: TheDartsScoreboardState, {score}) => {
    let arrows: string[] = [...state.currentDarts, score];
    arrows = arrows.length > 3 ? arrows.slice(arrows.length - 3):arrows;
    ...
    return {
      ...
      player1: {
        ...state.player1,
        doubleEndings: state.player1.currentPlayer? intermediateEnding(state, arrows, state.player1):state.player1.doubleEndings
      },
      player2: {
        ...state.player2,
        doubleEndings: state.player2.currentPlayer? intermediateEnding(state, arrows, state.player2):state.player2.doubleEndings
      }

    };
  }),
  on(clearCurrentDartSet, (state: TheDartsScoreboardState) => ({
    ...state,
    currentDarts: [],
    player1: {
      ...state.player1,
      doubleEndings: state.player1.currentPlayer? intermediateEnding(state, [], state.player1):state.player1.doubleEndings
    },
    player2: {
      ...state.player2,
      doubleEndings: state.player2.currentPlayer? intermediateEnding(state, [], state.player2):state.player2.doubleEndings
    }
  })),
